...
/**
 * LeafParticle — OOJS class that drives the 3D CSS floating leaf animation.
 * Uses CSS custom properties + requestAnimationFrame to create a live 3D rotation.
 * Demonstrates: 3D CSS (perspective, transform-style, rotateX/Y/Z) + OOJS pattern.
 */
class LeafParticle {
  ...
  _animate() {
    ...
    this.rotY  += 0.6;
    this.rotZ  += 0.2;
    this.floatY += 0.03 * this.floatDir;
    if (this.floatY >  8 || this.floatY < -8) this.floatDir *= -1;
...
// Init LeafParticle on DOM ready
document.addEventListener('DOMContentLoaded', () => {
  window._leafParticle = new LeafParticle('leafContainer');
});
